  ...
  function(self)
    ...
    if not isSampfuncsConsoleActive() and not sampIsChatInputActive() and not isPauseMenuActive() and not sampIsDialogActive() then
      ...
      if isKeyJustPressed(VK_J) then
        sampSendChat('/ocolor '..mp.lastObject..' 0 0xFFFFFFFF') -- last object blackout
      end

      if isKeyJustPressed(VK_B) then
        if not lastObject then
          sampSendChat('/oedit '..mp.lastObject) -- last object oedit
        end
      end

      if isKeyJustPressed(VK_M) then 
        sampSendChat('/odell') -- mouse odell
      end
      ...
      if isKeyJustPressed(VK_E) then -- create clipboard object
        if getClipboardText() == nil or string.len(getClipboardText()) > 5 then
          return
        end
